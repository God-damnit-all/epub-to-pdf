...
function unzipEpub(epubPath, extractDir) {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      try {
        execSync(`powershell -Command "Expand-Archive -Path '${epubPath}' -DestinationPath '${extractDir}' -Force"`, { stdio: 'inherit' });
      } catch (e) {
        // Fallback to 7zip or unzip
        try {
          execSync(`7z x "${epubPath}" -o"${extractDir}" -y`, { stdio: 'inherit' });
        } catch (e2) {
          execSync(`unzip -o "${epubPath}" -d "${extractDir}"`, { stdio: 'inherit' });
        }
      }
    } else {
      // Linux/Mac - use unzip
    ...
    console.log("💡 Make sure you have 'unzip' command installed on your system");
    return false;
  }
}
...
function findContentDirectory(extractDir) {
  ...
  for (const dirPath of possiblePaths) {
    if (fs.existsSync(dirPath)) {
      ...
      const hasXhtml = files.some(f => f.endsWith('.xhtml') || f.endsWith('.html'));
      if (hasXhtml) {
...
async function epubToPdf(contentDir, outputPdf) {
  ...
  console.log(`📄 Found ${files.length} content files:`, files.map(f => f.substring(0, 30) + (f.length > 30 ? '...' : '')));
  ...
  const exactWidth = viewportInfo.viewport.width || viewportInfo.body.width || 595;
  const exactHeight = viewportInfo.viewport.height || viewportInfo.body.height || 842;
  ...
  console.log(`✅ PDF saved to ${outputPdf} (${Math.round(stats.size / 1024)} KB)`);
  
  // Verify PDF dimensions
  const finalPdf = await PDFDocument.load(pdfBytes);
  const firstPage = finalPdf.getPage(0);
  const { width: pdfWidth, height: pdfHeight } = firstPage.getSize();
  console.log(`📏 Final PDF dimensions: ${Math.round(pdfWidth)}x${Math.round(pdfHeight)}`);
}
...
async function convertMultipleEpubs(epubDirectory, outputDirectory = null) {
  const epubFiles = fs.readdirSync(epubDirectory).filter(f => f.toLowerCase().endsWith('.epub'));
